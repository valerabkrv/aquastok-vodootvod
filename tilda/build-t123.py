#!/usr/bin/env python3
"""
Сборка сайта «Водоотвод Aquastok» в код для блока T123 (HTML-код) Тильды.

    python3 tilda/build-t123.py index.html --out tilda/aquastok-t123.html \
        --scope aq --base https://valerabkrv.github.io/<репозиторий>/

Сайт собран из внешних файлов (assets/style.css, assets/site.js, assets/data.js),
поэтому:
  ...
  * data.js и site.js остаются внешними и лежат на GitHub Pages — вместе
    с CSS они не влезли бы в лимит блока (100 000 байт);
  ...
  * пути к картинкам внутри data.js относительные, а страницу отдаёт Тильда,
    поэтому блок заранее объявляет window.AQ_ASSET_BASE — site.js склеивает
...
def build(src_path, scope, base):
    ...
    loader = (
        ...
        '  s.src = src;\n'
        '  s.onload = next;\n'
        '  s.onerror = function () { console.error("Aquastok: не загрузился " + src); };\n'
    ...
    note = ('<!-- Водоотвод Aquastok — блок T123 для Тильды.\n'
            ...
            '     Цены правятся прямо здесь — блок AQ_PRICES в самом низу. -->\n' % base)

    boot = ('<script>window.AQ_ASSET_BASE = "%s";</script>\n' % base)
...
def check(out, scope, base):
    ...
    for need in ('function load(', 'function start(', 'renderCatalog', 'AQ_PRICES'):
        if need not in out:
...
def main():
    ...
    ap.add_argument('--scope', default='aq')
